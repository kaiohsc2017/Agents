...
description: Orquestra o ciclo fechado de desenvolvimento seguro no AgentIA — planejamento com threat model leve, implementação, revisão de código+segurança em paralelo, verificação e registro de aprendizado em memória. Não traz checklists novos: aponta para as rules e agentes que o projeto já tem (.claude/rules/common/*.md, agentes ecc:*, sistema de memória). Use quando o usuário pedir para "planejar e implementar com segurança", "revisar isso com foco em segurança", ou "fechar o ciclo" de uma feature/fix antes de commitar.
---

# Secure Dev Loop (versão local, AgentIA)

Esta skill **não substitui** `.claude/rules/common/development-workflow.md`, `code-review.md`, `security.md` e `testing.md` — ela só garante que as fases sejam seguidas em ordem, com os agentes certos para o stack real deste projeto (Java/Spring, Python asyncio, React/TS, Docker/infra), e que o ciclo termine com aprendizado gravado em memória.

## Fase 0 — Contexto e sensibilidade do domínio

Antes de planejar, identifique:
- **Qual stack o pedido toca**: `backend/` (Java/Spring), `ai-agent/`/`agents-platform/backend/` (Python), `frontend/` (React/TS), `asterisk/`/`docker-compose.yml`/`Caddyfile` (infra).
- **Se toca superfície sensível** — a lista de gatilhos de `.claude/rules/common/code-review.md` ("Security Review Triggers"): auth/JWT/RBAC, input de usuário, queries, filesystem, chamadas externas (Zabbix/Gemini/Telegram), cripto. Se sim, a Fase 3 é **obrigatória com `ecc:security-reviewer`**, não opcional.
- **Se já existe plano relevante** em `.claude/plans/*.plan.md` — carregue antes de replanejar do zero.

## Fase 1 — Planejamento (brainstorm + plano bite-sized)

Para qualquer coisa não trivial, use a skill `ecc:plan` (ou `Agent` com `ecc:planner`/`ecc:architect` para decisão arquitetural). Force estas perguntas específicas do domínio AgentIA antes de aceitar o plano, quando aplicável:
...
- Se toca integração externa (Zabbix/Gemini/Telegram): há risco de SSRF/vazamento de credencial em mensagem de erro?
- Se toca migration: `V*.sql` é irreversível em produção — o SQL foi revisado a mão antes de escrever?
- Se é feature nova visível ao usuário: **release notes em `frontend/src/data/releases.ts` faz parte do plano**, não é opcional (`agentia_release_notes_mandatory`).
...
| Python (`ai-agent/`, `agents-platform/backend/`) | `ecc:python-reviewer` (+ `ecc:fastapi-reviewer` se FastAPI) | `ecc:security-reviewer` |
| React/TS (`frontend/`) | `ecc:react-reviewer` + `ecc:typescript-reviewer` | `ecc:security-reviewer` |
...
- Python: `python3 -m py_compile` nos arquivos tocados, `pytest` se houver suíte relevante
...
Não crie pasta `learnings/` separada — este projeto já tem sistema de memória (`/root/.claude/projects/-opt-AgentIA/memory/`). Ao final do ciclo:
